...
const sidebarVariants = cva(
  "fixed inset-y-0 left-0 z-50 flex w-[250px] flex-col bg-background shadow-lg transition-transform duration-300 ease-in-out md:static md:translate-x-0",
  {
    variants: {
      open: {
        true: "translate-x-0",
...
export function Sidebar({ className, ...props }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={cn(sidebarVariants({ open }), className)} {...props} />
    </>
  )
}
...
export function SidebarMenuButton({ className, isActive, tooltip, asChild = false, ...props }: SidebarMenuButtonProps) {
  ...
  return (
    <div className="relative group">
      ...
      {tooltip && (
        <div className="absolute left-full top-1/2 z-50 ml-2 -translate-y-1/2 rounded bg-primary px-2 py-1 text-xs text-primary-foreground opacity-0 transition-opacity group-hover:opacity-100">
          {tooltip}
        </div>
      )}
    </div>
